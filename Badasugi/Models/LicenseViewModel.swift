...
class LicenseViewModel: ObservableObject {
    ...
    // 개발자 모드: DEBUG 빌드에서 자동 활성화 + 환경 변수 지원
    private var isDeveloperMode: Bool {
        #if DEBUG
        // DEBUG 빌드에서는 자동으로 개발자 모드 활성화
        // 일반 사용자는 RELEASE 빌드를 사용하므로 안전함
        return true
        #else
        // RELEASE 빌드에서는 환경 변수나 특별한 키 필요
        // 개발자 테스트용 (일반 사용자는 모름)
        return ProcessInfo.processInfo.environment["BADASUGI_DEV_MODE"] == "true" ||
               userDefaults.string(forKey: "BadasugiDevModeKey") == "DEV-2024-BADASUGI-INTERNAL"
        #endif
    }
    ...
    var canUseApp: Bool {
        // DEBUG 빌드에서는 개발자 모드로 자동 활성화되어 항상 사용 가능
        // RELEASE 빌드에서는 정상적인 라이센스 체크 수행
        if isDeveloperMode {
            return true
        }
    ...
    var isLocked: Bool {
        // DEBUG 빌드에서는 개발자 모드로 자동 활성화되어 잠금 해제
        // RELEASE 빌드에서는 정상적인 라이센스 체크 수행
        if isDeveloperMode {
            return false
        }
