...
struct BadasugiApp: App {
    ...
    init() {
        ...
        // 자동 구두점 기능은 기본적으로 꺼져 있어야 함 (사용자가 명시적으로 켜지 않는 한)
        if UserDefaults.standard.object(forKey: "IsAutoPunctuationEnabled") == nil {
            UserDefaults.standard.set(false, forKey: "IsAutoPunctuationEnabled")
        }
            ...
            if hasCompletedOnboarding {
                ContentView()
                    ...
                    .onAppear {
                        ...
                        // 백그라운드에서 자동 업데이트 체크 (받아쓰기 업데이트만 체크)
...
// 받아쓰기 업데이트만 체크하도록 하는 Delegate
class BadasugiUpdaterDelegate: NSObject, SPUUpdaterDelegate {
    private let logger = Logger(subsystem: "com.badasugi.app", category: "Updater")
    
    func updater(_ updater: SPUUpdater, didFinishUpdateCheckFor updateCheck: SPUUpdateCheck, error: Error?) {
        // 업데이트 체크 완료 처리
        if let error = error {
            logger.error("업데이트 체크 중 오류 발생: \(error.localizedDescription)")
        }
    }
    
    func feedURL(for updater: SPUUpdater) -> URL? {
        // 받아쓰기(Badasugi)의 appcast만 사용하도록 명시적으로 지정
        let badasugiFeedURL = "https://raw.githubusercontent.com/Badasugi/badasugi/main/appcast.xml"
        guard let url = URL(string: badasugiFeedURL) else {
            logger.error("받아쓰기 appcast URL 생성 실패")
            return nil
        }
        logger.info("받아쓰기 업데이트 feed URL 사용: \(badasugiFeedURL)")
        return url
    }
}
...
class UpdaterViewModel: ObservableObject {
    ...
    private let updaterDelegate = BadasugiUpdaterDelegate()
    private let updaterController: SPUStandardUpdaterController
    
    @Published var canCheckForUpdates = false
    
    init() {
        // 이전에 저장된 VoiceInk 관련 feed URL이 있다면 제거 (안전을 위해)
        let userDefaults = UserDefaults.standard
        let feedURLKey = "SUFeedURL"
        if let existingFeedURL = userDefaults.string(forKey: feedURLKey) {
            // VoiceInk 관련 URL이면 제거
            if existingFeedURL.contains("voiceink") || existingFeedURL.contains("VoiceInk") {
                userDefaults.removeObject(forKey: feedURLKey)
            }
        }
        
        // 받아쓰기 업데이트만 체크하도록 delegate 설정
        ...
        // 자동 업데이트 체크 활성화 (받아쓰기 업데이트만 체크)
        updaterController.updater.automaticallyChecksForUpdates = autoUpdateCheck
        updaterController.updater.updateCheckInterval = 24 * 60 * 60 // 24시간마다 체크
    ...
    func toggleAutoUpdates(_ value: Bool) {
        // 자동 업데이트 설정 변경 (받아쓰기 업데이트만 체크)
        updaterController.updater.automaticallyChecksForUpdates = value
    }
    
    func checkForUpdates() {
        // 수동 업데이트 확인만 허용 (Badasugi 업데이트만 체크)
